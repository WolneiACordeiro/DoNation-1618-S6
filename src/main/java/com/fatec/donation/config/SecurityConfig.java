    ...
    SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity, JwtFilter jwtFilter) throws Exception {
        return httpSecurity
                ...
                .authorizeHttpRequests(auth -> {
                    ...
                    auth.requestMatchers(HttpMethod.PUT, "/api/v1/groups/block/{groupId}/{blockedUserId}").hasAnyRole("ADMIN", "USER");
                    auth.requestMatchers(HttpMethod.PUT, "/api/v1/groups/unblock/{groupId}/{unblockedUserId}").hasAnyRole("ADMIN", "USER");
